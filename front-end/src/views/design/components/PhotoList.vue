 ...
 * @LastEditTime: 2022-02-21 23:46:39
...
<template>
  ...
  <div class="imgs__wrap">
    <el-card v-for="(item, i) in listData" :key="i + 'i'" shadow="hover" :body-style="{ padding: '0px', position: 'relative' }">
      <el-image class="cover-box" :src="item.thumb" :preview-src-list="[item.url]" fit="cover"></el-image>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const load = async () => {
      ...
      const res = await api.getPhotoList({ cate: props.cate, page: state.page.index, order: 'asc' })
