...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const load = async () => {
      ...
      const res = await api.getTempList({ cate: props.cate, page: state.page.index, state: state.state, type: 0, order: 'asc' })
    ...
    const turnState = async (item: any) => {
      ...
      await api.saveTemp({ id: item.id, state: item.state })
