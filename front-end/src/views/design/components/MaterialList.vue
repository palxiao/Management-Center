 ...
 * @LastEditTime: 2022-02-22 14:06:56
...
<template>
  ...
  <div class="imgs__wrap">
    <el-card v-for="(item, i) in listData" :key="i + 'i'" shadow="hover" :body-style="{ padding: '0px', position: 'relative' }">
      <el-image class="cover-box" :src="item.thumb" :preview-src-list="[item.thumb]" fit="contain"></el-image>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const load = async () => {
      ...
      const res = await api.getMaterialList({ cate: props.cate, page: state.page.index, state: state.state, order: 'asc' })
    ...
    const turnState = async (item: any) => {
      ...
      await api.updateMaterial({ id: item.id, state: item.state })
